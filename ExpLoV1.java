...
public class ExpLoV1 {
    ...
    public void ExpLogica(String CadLog) {
        ...
        if (Elog != "0") {
            ...
            for (int i = 0; i < Cifras.length; i++) {
                if (Cifras[i].matches("^\\¬$") || Cifras[i].matches("^\\^$") || Cifras[i].matches("^v$") || Cifras[i].matches("^->$") || Cifras[i].matches("^<->$")) {
                    nn++;
                }
            }
    ...
    public static String OrdenPost(String exp) {
        ...
        for (int i = 0; i < Exp.length; i++) {
            if (Exp[i].matches("^[A-Z]{1}$")) {//Variable
                ...
                Presedencia = 3;
            } else if (Exp[i].matches("^[\\^||v]$")) {//AND y OR
                if (Presedencia == 2) {
                    operacion.push(operadores.pop());
                    operadores.push(Exp[i]);
